...
function getTargetType(value: Target) { // 如果此值不需要代理， 或者这个对象不能被扩展
...
function createReactiveObject(
  ...
  // exception: calling readonly() on a reactive object 如果被reactive处理过的对象还可以继续被readonly处理
  if (
    target[ReactiveFlags.RAW] && 
    !(isReadonly && target[ReactiveFlags.IS_REACTIVE]) // readonly(reactive(obj))
  ) {
    return target
  }
  // target already has corresponding Proxy   检测目标是不是已经被代理过了
