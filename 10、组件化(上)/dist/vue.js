(function (global, factory) {
    ...
    function createComponent$1(vm, tag, data, children, key, Ctor) {
      ...
      data.hook = {
        ...
        init(vnode) {
          ...
          child.$mount(); //mount挂载完毕后,会产生一个真实节点 这个节点在实例上vm.$el,所以可以把这个实例保存在vnode的一个属性上这里是compoentInstance
        },
    ...
    function patch(oldVnode, vnode) {
      if (!oldVnode) {
        //说明是组件的挂载
        return createEle(vnode); //产生一个组件的真实节点
    ...
    function createComponent(vnode) {
      ...
      if ((i = i.hook) && (i = i.init)) {
        i(vnode);
      } //这个判断是上面i(vnode)函数执行后的条件 上面执行的时候会给vnode增加compoentInstance


      if (vnode.compoentInstance) {
        return true; //说明是组件
      }
    ...
    function createEle(vnode) {
      ...
      if (typeof tag === "string") {
        ...
        if (createComponent(vnode)) {
          return vnode.compoentInstance.$el;
        }
    ...
    function initGlobalAPI(Vue) {
      ...
      Vue.extend = function (opt) {
        ...
        Sub.mixin = Vue.mixin; //....还有很多 不一一列举
