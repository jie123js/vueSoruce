...
Layer.prototype.match = function (pathname) {
    ...
    if (this.route) { // 路由中可能是模糊匹配的 
        let matches = pathname.match(this.regexp); // /school/1/2  1 2

        if(matches){
            let [,...args] = matches
            this.params  = this.keys.reduce((memo, current, index) => (memo[current.name] = args[index], memo), {})
            return true
        }else{
            return false
        }
